...
select no, name, description, by_category.count from category join (select count(*) as count, category_no from post group by category_no) by_category where id = 'yujin0419' and by_category.category_no = category.no;